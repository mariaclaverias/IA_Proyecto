...
public class DataViewAttack : DataViewBase
{
    ...
    public override bool IsInSight(Transform AimOffset)
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(origin, (dest - origin).normalized, out hit, direcction.magnitude, Scanlayers))
        {
            meshColor = Color.red;
            meshColor.a = 0.4f;
            Attack = true;
    ...
    public override void OnDrawGizmos()
    {
        ...
        //if (ee != null)
        //{
        //    Vector3 direcction = ee.AimOffset.position - Owner.AimOffset.position;
        //    Gizmos.DrawLine(Owner.AimOffset.position, Owner.AimOffset.position + direcction.normalized * (direcction.magnitude));
        //    //Gizmos.DrawWireSphere(Owner.AimOffset.position + direcction.normalized * (distance), radiusSphereCast);
        //}
...
public class DataViewFire : DataViewBase
{
    ...
    public override void OnDrawGizmos()
    {
        ...
        //{
        //    Vector3 direcction = ee.AimOffset.position - Owner.AimOffset.position;
        //    Gizmos.DrawLine(Owner.AimOffset.position, Owner.AimOffset.position + direcction.normalized * (distance));
        //    //Gizmos.DrawWireSphere(Owner.AimOffset.position + direcction.normalized * (distance), radiusSphereCast);
        //}
    ...
    private void ExtractViewEnemy(ref float min_dist, Health _health)
    {
        if (!IsAllies(_health))
        {
            ...
            if (min_dist > dist)
            {
                ViewEnemy = _health;
                min_dist = dist;
                Memory = ViewEnemy.transform.position;
            }
            CountEnemyView++;
        }
        else
